...
// --- NUEVAS FUNCIONES DE RESETEO DE CONTRASEÑA ---
// @desc    Solicitar reseteo de contraseña (enviar email)
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    console.log(`Email no encontrado, enviando respuesta genérica.`);
  ...
  console.log(`Token guardado en BD para ${email}`);
  ...
  const baseUrl = process.env.NODE_ENV === 'production'
    ...
    console.log(`Base URL seleccionada para el email: ${baseUrl}`); // Log para verificar

  try {
    await sendEmail(
      user.email,
      'Restablece tu contraseña de CampoBit',
      emailTemplates.resetPassword(resetUrl, user.name)
    );
...
// @desc    Resetear la contraseña (usando el token)
...
const resetPassword = asyncHandler(async (req, res) => {
    ...
    console.log(`Intento de reseteo con token: ${token}`);

    // --- ⭐ VALIDACIÓN DE 6 CARACTERES AÑADIDA AQUÍ ⭐ ---
    if (!password || password.length < 6) {
        console.log('Error: La contraseña es muy corta.');
        return res.status(400).json({
            success: false,
    ...
    // 1. Hashear el token que viene de la URL
    ...
    // 4. Limpiar los campos de reseteo
    user.resetPasswordToken = undefined;
    user.resetPasswordExpire = undefined;

    // 5. Resetear intentos de login (por si estaba bloqueado)
